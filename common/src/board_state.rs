    ...
    T: Clone + Debug + Tile<Kind = K, TileConfig = C>
{
    ...
    pub fn place_tile(&mut self, loc: B::TLoc, tile: T) {
    ...
    pub fn advance_players(&mut self, board: &B, loc: B::TLoc) {
        ...
        let mut to_advance = (0..self.players.len())
            .flat_map(|i| self.player_port(i as u32)
                .and_then(|p| board.loc_ports(loc.clone()).into_iter().position(|q| p == q))
        ...
        while to_advance.iter_mut().map(|(player, maybe_loc)| {
                if let Some(loc) = maybe_loc {
                    ...
                    let input = board.loc_ports(loc.clone()).into_iter().position(|p| p == port_in).unwrap() as u32;
                    let tile = self.tile_at(loc.clone()).unwrap().clone();
                    let output = tile.output(input);
                    let port_out = board.loc_ports(loc.clone())[output as usize].clone();
